...
function confirm($result){
    global $connection;
    if (!$result) {
        die("QUERY FAILED AGAIN zucht " . mysqli_error($connection));
    }
}
...
// _______________________________________FRONTEND FUNCTIES_________________________________________


// functie voor alles te tellen van een onderdeel, dit moet eig bij helperfuncties, later, nu teveel werk.
/* (n1) function count_all_records(){
    $query = query(" SELECT * FROM products");
    confirm($query);
    return mysqli_num_rows($query);*/

/* (n2) function count_all_records($table){
...
function get_products_with_pagination($perPage = "6"){
    ...
    // (n3)$rows = count_all_records('products');
    $rows = count_All_Products_In_Stock();

    // (n1) $rows = count_all_records();
    /*$query = query(" SELECT * FROM products");
    confirm($query);
    ...
    /*DIT IS CODE OM " DIT IS PAGINA ZOVEEL VAN DE ZOVEEL "
     * if($lastPage != "1"){
...
DELIMETER;
  ...
  if (!$send_query){
      ...
      die("DE QUERY FAALDE AKA E FOUT IN DE QUERY" . mysqli_error($connection));
  }
...
/*
 testen of de connectie werkt
 if($connection) {
    echo "is connected <br>" . "<br>";
}
...
//DIT WERD VERVANGEN DOOR GET PRODUCTS WITH PAGINATION
...
if (isset($_POST['submit'])){
    ...
    if (!$result){
        set_message("Sorry we could not send your message. something went wrong");
...
//functie om uploads/{$row['product_image'] te vervangen

//manier 1
/*function discplay_image($picture){
...
<tr>
            ...
            <td><a href="index.php?edit_product&id={$row['product_id']}"><img width="100" src="../../resources/{$product_image}" alt=""></a>
            <br>{$row['product_title']}
              </td>
...
function add_products(){
    ...
    if (isset($_POST['publish'])){
        ...
        $product_author        = escape_string($_POST['product_author']);
        ...
        $query = query("INSERT INTO products(product_title, product_author, product_category_id, product_price, product_description, short_desc, product_quantity, product_image) VALUES('{$product_title}', '{$product_author}', '{$product_category_id}', '{$product_price}', '{$product_description}', '{$short_desc}', '{$product_quantity}', '{$product_image}')");
        $last_id = last_id();
        confirm($query);
        set_message(("New project with id {$last_id} was added"));
...
//query uitgetypt zoals in de les, minder efficient wel.
...
function add_category(){
    if (isset($_POST['add_category'])) {
        ...
        /*stuk code om te controlere of er data in de database wordt ingevoerd
        if (mysqli_affected_rows($insert_cat) == 0){
            set_message("nothing worked");
        }*/
        set_message("category created, congratz");
...
<img class="slide-image" width="800" height="300" src="../resources/{$slide_image}" alt="">
        </div>
DELIMETER;
...
function get_slides(){
    ...
    while ($row = fetch_array($query)){
        ...
        $slides = <<<DELIMETER
<div class="item">
<img class="slide-image " width="800px" height="300px" src="../resources/{$slide_image}" alt="">
        </div>
DELIMETER;
...
function get_products_from_search_in_search(){
    global $connection;

    if (isset($_POST['submit-search'])) {
        $search = mysqli_escape_string($connection, $_POST['search']);
        $sql = "SELECT * FROM products WHERE product_title LIKE '%$search%' OR product_author LIKE '%$search%'";
        $result = mysqli_query($connection, $sql);
...
/*function get_author_from_search_in_search(){
    global $connection;

    if (isset($_POST['submit-search'])) {
        $search = mysqli_escape_string($connection, $_POST['search']);
        $sql = "SELECT * FROM products WHERE product_author LIKE '%$search%' OR product_author LIKE '%$search%'";
        $result = mysqli_query($connection, $sql);
        $queryResult = mysqli_num_rows($result);
